...
if ( ! class_exists( 'Woostify_Variation_Swatches_Frontend' ) ) {
	...
	class Woostify_Variation_Swatches_Frontend {
		...
		public function get_swatch_html( $html, $args ) {
			...
			$url_variation_active = '1' === $settings['url_variation_active'] ? 'url-active' : '';
			...
			if ( array_key_exists( $attr->attribute_type, $swatch_types ) ) {
				...
				if ( ! empty( $swatches ) ) {
					...
					$swatches = '<div class="woostify-variation-swatches ' . esc_attr( $quickview ) . ' ' . esc_attr( $url_variation_active ) . ' ' . esc_attr( $product->get_stock_status() ) . ' variation-' . $settings['style'] . '" data-attribute_name="attribute_' . esc_attr( $attribute ) . '">' . $swatches . '</div>';
		...
		public function swatch_list() {
			...
			$check_stock = count( $attributes ) == 1;
			foreach ( $attributes as $key ) {
				...
				foreach ( $vars as $key ) {
					...
					if( $check_stock && $key['is_in_stock'] == false ){
						$_class .= ' unavailable';
					}
