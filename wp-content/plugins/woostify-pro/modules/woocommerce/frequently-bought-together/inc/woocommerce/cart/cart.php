	...
	<table class="shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
		<thead>
			<tr>
				...
				<th class="product-subtotal"><?php esc_html_e( 'Total', 'woostify-pro' ); ?></th>
					...
					foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
						...
						if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
							...
							<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); echo ' '.$product_bundle_class ?>">	
									...
									<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'woostify-pro' ); ?>">
										<?php
											...
											if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
												echo wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'woostify-pro' ) . '</p>' ) );

											}
										?>
									</td>
								<?php else:?>
									<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'woostify-pro' ); ?>" colspan="2">
										<?php
											if ( ! $product_permalink ) {
											...
											// Backorder notification
											if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
												echo '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'woostify-pro' ) . '</p>';
											}
										?>
									</td>
								<?php endif;?>
								...
								<?php endif;?>
								<td class="product-subtotal" data-title="<?php esc_attr_e( 'Total', 'woostify-pro' ); ?>">
									<?php
							...
							<?php if ( ! empty( $cart_item['bundle-products'] ) ) :?>
								<tr class="product-bundle-title">
									<td colspan="6">
										<h6><?php esc_attr_e( 'Product bundles', 'woostify-pro' );?></h6>
									</td>
								</tr>
							<?php endif;?>
							...
							<tr class="bundle-products bundle-off-<?php echo $product_id;?>">
								<td colspan="6">
									<?php
										...
										$bundles_added 		= explode( ',', $cart_item['bundle-products'] );//var_dump($bundles);exit;
										if ( $bundles ) {
											echo '<ul class="product-bundle">';
											foreach( $bundles as $key => $val ){
												if ( in_array( $val['id'], $bundles_added ) ) {
													...
													if ( isset( $custom_variable[$val['id']] ) && count( $custom_variable[$val['id']] ) > 0 ) {
														...
														echo '<span class="db" style="text-transform: capitalize;">';
															echo $custom_variable[$val['id']]['variable'];
														echo '</span>';
													} else {
														// Get variable
				...
				<td colspan="6" class="actions">
					...
					<?php if ( wc_coupons_enabled() ) { ?>
						<div class="coupon flex middle-xs">
							...
							<label for="coupon_code"><?php _e( 'Coupon:', 'woostify-pro' ); ?></label> <input type="text" name="coupon_code" class="input-text br__40" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Coupon code', 'woostify-pro' ); ?>" /> <button type="submit" class="button" name="apply_coupon" value="<?php esc_attr_e( 'Apply Coupon', 'woostify-pro' ); ?>"><?php esc_attr_e( 'Apply coupon', 'woostify-pro' ); ?></button>

							<?php do_action( 'woocommerce_cart_coupon' ); ?>
						</div>
					<?php } ?>

					<button type="submit" class="button update-cart" name="update_cart" value="<?php esc_attr_e( 'Update Cart', 'woostify-pro' ); ?>"><?php esc_attr_e( 'Update Cart', 'woostify-pro' ); ?></button>
