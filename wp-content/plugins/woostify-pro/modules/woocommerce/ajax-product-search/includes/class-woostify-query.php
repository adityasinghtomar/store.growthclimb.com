...
class Woostify_Query {
	...
	public function __construct( $args ) {
		$this->keyword                  = str_replace("'","\\'", $args['keyword'] );
		...
		$this->search_custom_field      = $args['search_custom_field'];
	}
	...
	protected function query_string() {
		...
		$custom_field      = $this->parse_custom_field( $keyword );
		...
		if ( $this->search_custom_field ) {
			$sql .= " INNER JOIN {$wpdb->prefix}postmeta as acf ON tproduct.id = acf.post_id";
		}
	...
	/**
	 * Parse search product custom fields.
	 *
	 * @param (string) $keyword | Keyword search product custom field.
	 * @return (string) sql search product description.
	 */
	protected function parse_custom_field( $keyword ) {
		$custom_fields  = $this->search_custom_field;
		$key            = $keyword;
		$keywords       = explode( ' ', $key );
		$length         = count( $keywords );
		$metakey_string = "'" . implode( "', '", explode( ',', $custom_fields ) ) . "'";
		if ( $this->search_custom_field && ! empty( $custom_fields ) ) {
			$sql = "( acf.meta_key in ( $metakey_string )"; // Du 1 dau mo ngoac don.
			foreach ( $keywords as $index => $key ) {
				$sql .= " AND acf.meta_value LIKE '%$key%'";
			}
			$sql .= ')'; // Close dau ngoac don.
	...
	public function tags_query_string() {
		...
		$sql      = '';
		if ( ! empty( $keywords ) ) {
			$sql = "SELECT * FROM {$wpdb->prefix}woostify_tag_index WHERE ";
	 ...
	 * @return Product.
	 */
	protected function get_product_thumbnail_url( $product ) {
		global $wpdb;

		$image_size = get_option( 'woostify_ajax_search_product_thumbnail_size' );
		$product_id = $product->id;
		$image_url = $product->image;

		$p_sql = "SELECT p.ID, exm.meta_value AS image_url, exdata.meta_value AS image_metadata FROM `wp_posts` p
		LEFT JOIN wp_postmeta ex ON p.ID=ex.post_id AND ex.meta_key = '_thumbnail_id'
		LEFT JOIN wp_postmeta exm ON ex.meta_value=exm.post_id AND exm.meta_key='_wp_attached_file'
		LEFT JOIN wp_postmeta exdata ON ex.meta_value=exdata.post_id AND exdata.meta_key='_wp_attachment_metadata'
		WHERE p.post_status='publish' AND p.post_type='product' AND  p.ID = $product_id";

		$results = $wpdb->get_results( $p_sql );
		$image_metadata_unserialize = '';
		$product_images = [];

		if( !empty($results) && !empty($results[0]->image_metadata) ){
			
			$image_metadata = $results[0]->image_metadata; 
			$image_metadata_unserialize = unserialize($image_metadata);
			$upload_dir  = '/wp-content/uploads'; 
			$file = $image_metadata_unserialize['file'];
			$file_arr = explode("/",$file);
			$folder = $file_arr[0].'/'.$file_arr[1];  
			$sizes = $image_metadata_unserialize['sizes'];
			if( !empty( $sizes ) ){
				if( $image_size == 'full'){
					$product_images['full'] = $upload_dir . '/' . $file;
				}
				foreach ($sizes as $s_key => $item ) {
					$path = $upload_dir . '/' . $folder . '/' . $item['file'];
					$product_images[$s_key] = $path;	
				}
			}

		}

		if( !empty( $product_images ) && isset($product_images[$image_size]) ){
			$image_url = $product_images[$image_size];
		}
	...
	protected function result_product() {
		...
		if ( ! empty( $products ) ) {
			$bk_keyword = $this->keyword;
			$this->keyword = str_replace( "\\'", "'", $this->keyword );
			foreach ( $products as $product ) {
				$thumbnail_url = $this->get_product_thumbnail_url( $product );
				...
				if ( strpos( $this->search_custom_field, '_price' ) !== false ) {
					$product->html_price = $this->highlight( $product->html_price );
				}
			}
			$this->keyword = $bk_keyword;
