...
if ( ! class_exists( 'Woostify_Ajax_Product_Search' ) ) :
	...
	class Woostify_Ajax_Product_Search {
		 ...
		 * @param (array) $flag_tag | product highline tag.
		 */
		public function search_key( $string, $key, $title, $flag_tag ) {
			...
			$name         = str_ireplace( $key, str_repeat($flag_tag[0], 5) . $text . str_repeat($flag_tag[1], 5), $pre_key );
			if ( $count > 1 ) {
				$name .= $this->search_key( $sub_key, $key, $title, $flag_tag );
			}
		...
		/**
		 * Get tag key to replace to highline
		 *
		 * @param (int) $string | product id.
		 */
		public function get_tag_key($input) {
			$flag_key = array('-', '+', '!', '@', '~', '#', '%', '^', '&', '*');
			$flag_tag = array();
			foreach ($flag_key as $key) {
		...
		public function highlight( $string, $keyword ) {
			...
			if ( count( $args ) > 1 ) {
				$flag_tag = $this->get_tag_key($string);
				$flag_tag = !empty($flag_tag) ? $flag_tag : array('-', '+');
				foreach ( $args as $key ) {
					...
					if ( $count > 1 ) {
						$pre  = str_ireplace( $key, str_repeat($flag_tag[0], 5) . $text . str_repeat($flag_tag[1], 5), $pre_key );
						$t    = $this->search_key( $sub_key, $key, $str, $flag_tag );
						$name =  $pre . $t; //phpcs:ignore
						$str  = $name;
					} else {
						$name = str_ireplace( $key, str_repeat($flag_tag[0], 5) . $text . str_repeat($flag_tag[1], 5), $str );
						$str  = $name;
					}
				}

				$name = str_ireplace( str_repeat($flag_tag[0], 5), '<span class="aps-highlight">', $name );
				$name = str_ireplace( str_repeat($flag_tag[1], 5), '</span>', $name );
		...
		public function add_setting_url() {
			$position = 5;
			$sub_menu = add_submenu_page( 'woostify-welcome', 'Settings', __( 'Ajax Product Search', 'woostify-pro' ), 'manage_options', 'ajax-search-product-settings', array( $this, 'add_settings_page' ), $position );
		}
		...
		public function register_settings() {
			...
			register_setting( 'ajax-search-product-settings', 'woostify_ajax_search_product_thumbnail_size' );
		}
		...
		public function get_options() {
			...
			$options['search_product_custom_field'] = get_option( 'woostify_ajax_search_product_custom_field', '' );
			$options['product_thumbnail_size'] 		= get_option( 'woostify_ajax_search_product_thumbnail_size', 'thumbnail' );
		...
		public function add_settings_page() {
			...
			$thumbnail_sizes = woostify_display_all_thumbnail_sizes();
			?>
						...
						<div class="woostify-settings-content">
							...
							<div class="woostify-settings-section-content">
								<table class="form-table">
											...
											<div class="woostify-add-custom-field">
												<div class="custom-field-wrapper">
													<div class="woostify-custom-field-control">
														<?php
														// Draw list selected.
														if ( ! empty( $options['search_product_custom_field'] ) ) {
															$custom_field_selected = explode( ',', $options['search_product_custom_field'] );
															foreach ( $custom_field_selected as $field ) {
																?>
																<span class="woostify-auto-complete-key">
																	<span class   ="woostify-title">
																		<?php echo esc_html( $field ); ?>
																	</span>
																	<span class="btn-woostify-auto-complete-delete ion-close" data-item="<?php echo esc_html( $field ); ?>"></span>
																</span>
																<?php
															}
														}
														...
														<input type="hidden" id="woostify_ajax_search_product_custom_field" name="woostify_ajax_search_product_custom_field" value="<?php echo esc_attr( $options['search_product_custom_field'] ); ?>">
													</div>
									...
									<tr>
										<th><?php esc_html_e( 'Product Thumbnail', 'woostify-pro' ); ?>:</th>
										<td>
											<label for="woostify_ajax_search_product_thumbnail_size">
												<select name="woostify_ajax_search_product_thumbnail_size" id="woostify_ajax_search_product_thumbnail_size">
													<?php foreach ($thumbnail_sizes as $s_key => $size ) {
														$size_name = ucwords($size['name']) . ' - ' .$size['width'] . 'x' . $size['height'] ;
														?>
														<option <?php selected( $options['product_thumbnail_size'], $s_key ); ?> value="<?php echo $s_key; ?>">
															<?php esc_html_e( $size_name, 'woostify-pro' ); ?>
														</option>
														<?php
													} ?>
													<option <?php selected( $options['product_thumbnail_size'], 'full' ); ?> value="full">
														<?php esc_html_e( 'Full', 'woostify-pro' ); ?>
													</option>
												</select>
											</label>
										</td>
		 ...
		 * Remove by dev. Keep a backup until next version
		 */
		...
		public function untrash_post( $post_id ) {
			...
			if ( 'product' == get_post_type( $post_id ) ) { //phpcs:ignore
				...
				$index->create_product( $post );
			}
		}

		/**
		 * Update table index when save a product.
		 *
		 * @param (string|int) $post_id | Post Id.
		 * @param (object) $post | WP_Post object.
		 * @param (boolean) $is_update | true: update an exxisting product, false: create new
		 */
		public function save_post_product( $post_id, $post, $is_update ) {
			$this->create_product( $post_id, $post, $is_update );
		}

		/**
		 * Update table index after create new product
		 *
		 * @param (string|int) $post_id }| Post Id.
		 */
		public function create_product( $post_id = null, $post=array() , $is_update=false ){
			// error_log(print_r( array( 'create_product: Update table index after create new product', $post_id, $post)));
			// var_dump( func_get_args() ); exit;
			if( empty( $post_id ) || $is_update  ) {
				return;
			}
			if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
				return;
		...
		public function save_product( $post_id, $post ) {
			if( empty( $post_id ) || $post->post_type !== 'product') {
				return;
			}
			$index      = new Woostify_Index_Table();
			$index->create_product( $post );
