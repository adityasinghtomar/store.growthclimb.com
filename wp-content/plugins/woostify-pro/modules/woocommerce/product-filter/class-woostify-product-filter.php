...
if ( ! class_exists( 'Woostify_Product_Filter' ) ) {
	...
	class Woostify_Product_Filter {
		...
		public function __construct() {
			...
			// Pre get posts.
			add_action( 'pre_get_posts', array( $this, 'filter_pre_get_posts' ), 50 );
		...
		public function woostify_hide_all_noticee_page_setting() {
			...
			if ( $screen->id == 'woostify_page_smart-product-filter-settings' ) {
		...
		/**
		 * Insert database multi-row in the same time
		 *
		 * @param array $args The args.
		 */
		public function insert_multi( $prepare_products = array() ) {
			require_once ABSPATH . 'wp-admin/includes/upgrade.php';
		...
		public function get_filters_url() {
			...
			$sql = "SELECT p.ID FROM $table_posts p";
			if ( defined( 'ICL_SITEPRESS_VERSION' ) && defined( 'ICL_LANGUAGE_CODE' ) ) {
				$table_translations = $wpdb->prefix . 'icl_translations';
				$sql .= " JOIN $table_translations t ON p.ID = t.element_id";
			}
			$sql .= " WHERE p.post_type = 'product_filter' AND p.post_status = 'publish'";
			if ( defined( 'ICL_SITEPRESS_VERSION' ) && defined( 'ICL_LANGUAGE_CODE' ) ) {
				$table_translations = $wpdb->prefix . 'icl_translations';
				$sql .= " AND t.language_code = '" . ICL_LANGUAGE_CODE . "'";
			}
			...
			foreach ( $data as $k => $v ) {
				...
				switch ( $type ) {
					case 'search':
						...
						$filter_key .= '<span class="w-filter-key-remove" data-id="' . $k . '" data-type="search" 	data-value="' . $v . '">' . sprintf( /* translators: keyword */__( '%1$s: %2$s', 'woostify-pro' ), $label, $v ) . '<span class="w-filter-key-remove-icon">' . $remove_icon . '</span></span>';
						break;
					case 'date-range':
						if ( $boolean ) {
							$v = explode( ',', $v );
						...
						$filter_key .= '<span class="w-filter-key-remove" data-id="' . $k . '" data-type="date-range" data-value="' . $v[0] .','. $v[1] .'">' . sprintf( /* translators: date range */__( '%1$s: %2$s - %3$s', 'woostify-pro' ), $label, $v[0], $v[1] ) . '<span class="w-filter-key-remove-icon">' . $remove_icon . '</span></span>';
						break;
					case 'check-range':
					...
					case 'range-slider':
						...
						if ( isset( $v[0] ) && isset( $v[1] ) ) {
							...
							$filter_key .= '<span class="w-filter-key-remove" data-id="' . $k . '" data-type="range-slider" data-value="' . $v[0] . ',' . $v[1] . '">' . sprintf( /* translators: range slider */__( '%1$s: %2$s - %3$s', 'woostify-pro' ), $label, $slider_from, $slider_to ) . '<span class="w-filter-key-remove-icon">' . $remove_icon . '</span></span>';
						}
						break;
					case 'rating':
						$args['meta_query'][] = array(
						...
						$filter_key   .= '<span class="w-filter-key-remove" data-id="' . $k . '" data-type="rating" data-value="' . $v . '">' . sprintf( /* translators: rating */__( '%1$s: %2$s', 'woostify-pro' ), $label, $rating_text ) . '<span class="w-filter-key-remove-icon">' . $remove_icon . '</span></span>';
						break;
					case 'sort-order':
						...
						$filter_key .= '<span class="w-filter-key-remove" data-id="' . $k . '" data-type="sort-order" data-value="' . $v . '">' . sprintf( /* translators: rating */__( '%1$s: %2$s', 'woostify-pro' ), $label, $catalog_orderby_options[$v] ) . '<span class="w-filter-key-remove-icon">' . $remove_icon . '</span></span>';
						break;
					case 'stock':
		...
		// check and get condition of product filter for main query
		public function get_filters_condition($query) {
			global $wpdb;

			$filter_ids = $wpdb->get_col(
				"SELECT ID
				FROM $wpdb->posts 
				WHERE post_status = 'publish' AND post_type = 'product_filter'"
			);
			...
			if ( $filter_ids ) {
				...
				foreach ($filter_ids as $key => $filter_id) {
					...
					$woostify_filter_data = get_post_meta( $filter_id, 'woostify_product_filter_data', true );
					
					if($condition_select && $condition_field){
						$condition_field = explode( PHP_EOL, $condition_field );
						$term_arr = array();
						foreach ( $condition_field as $name ) {
							$condi_term = get_term_by( 'name', $name, $woostify_filter_data );
							if ( $condi_term ) {
								$term_arr[] = $condi_term->term_id;
							}
						}
						$condition = ( $condition_select == 'exclude' )? 'NOT IN' : 'IN';
						if( taxonomy_exists( $woostify_filter_data ) ){
							$tax_query[] = array(
								'taxonomy' => $woostify_filter_data,
								'field'		 => 'term_id',
								'terms'    => $term_arr,
								'operator' => $condition,
							);
		...
		public function filter_pre_get_posts( $query ) {
			...
			if ( is_shop() && !is_admin() ) {
				...
				$_page = max( 1, $this->get_paged_from_request() );
				
				// $this->get_filters_condition( $query );
		...
		public function maybe_index_hierarchical_filter( $post_id = false ) {
			global $wpdb;
			$prefix = $wpdb->prefix;
			// Hien tai moi chi ho tro product_cat nen code cứng phan nay
			...
			foreach( $hierarchical_filter as $taxonomy ){
				if( empty( $hierarchies[ $taxonomy ] ) ) {
					$hierarchies[ $taxonomy ] = $this->get_term_depths( $taxonomy );
				}
			}
			foreach ( $hierarchies as $taxonomy_name => $t_hierarchies ) {
				// Get all product ids available.
				if( $post_id ){
					$has_product_ids = array( $post_id );
				} else {
					$has_product_ids = $this->get_product_ids_by_term( $taxonomy_name ); // 0.17s
				}
				if ( empty( $has_product_ids ) || ( $post_id && ! in_array( $post_id, $has_product_ids ) ) ) {
					continue;
				}
				$args = array(
					'post_type' => 'product',
					'post__in' => $has_product_ids,
					'posts_per_page' => -1,
				);
				...
				if ($query->have_posts()) {
					while ($query->have_posts()) {
						...
						$product_hierarchies = get_the_terms($product_id, $taxonomy_name);
						// Params.
						$params     = array(
							'product_id' => get_the_ID(),
							'title'      => htmlentities( get_the_title( ), ENT_QUOTES ),
							'price'      => get_post_meta($product_id, '_price', true),
							'sku'        => htmlentities( get_post_meta($product_id, '_sku', true), ENT_QUOTES ),
							'rating'     => get_post_meta($product_id, '_rating', true),
						);

						if (!empty($product_hierarchies)) {
							foreach( $product_hierarchies as $product_hierarchy ){
								$parent_id = 0;
								$product_hierarchy = (array) $product_hierarchy;
								do {
									$term_id = $product_hierarchy['term_id'];
									$depth   = $t_hierarchies[ $term_id ][ 'depth' ];
									$k       = $product_id . '_' . $term_id;

									// Row item
									$hf_params              = $params;
									$hf_params['term']      = $taxonomy_name;
									$hf_params['term_name'] = $product_hierarchy['name'];
									$hf_params['term_id']   = $product_hierarchy['term_id'];
									$hf_params['parent_id'] = $product_hierarchy['parent'];
									$hf_params['depth']     = $depth;

									// key này không sử dụng, đưa thêm cho đủ column của DB
									$hf_params['variation_id'] = 0;

									// Add to an array and save late
									$prepare_products[ $k ] = $hf_params; // Lấy key để tránh lặp

									$parent_id = $product_hierarchy['parent'];
									if( $parent_id ) {
										$product_hierarchy = $t_hierarchies[$parent_id];
									}
								} while ( $parent_id ); // lấy ngược lên các item cha
							}
						}
					}
				}

				wp_reset_postdata(); // Thiết lập lại dữ liệu bài viết sau khi truy vấn kết thúc

			}
			// Save Data
			$this->insert_multi( $prepare_products );
		}
			...
			if ( ! empty( $product_variation_ids ) ) {
				...
				if ( ! empty( $_metas ) ) {
					foreach ( $_metas as $_meta ) {
						...
						$p_meta[ $_meta['post_id'] ][ $_meta['meta_key'] ] = $_meta['meta_value']; // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
					}
					unset( $_metas );
				}
		...
		public function add_admin_menu() {
			...
			if ( $submenu['woostify-welcome'] ) {
				foreach ( $submenu['woostify-welcome'] as $key => $value ) {
					if ( $value[2] == 'edit.php?post_type=product_filter' ) {
						$submenu['woostify-welcome'][$key][4] = "callback setting";
					}
				}
			}
		}
		...
		public function get_options() {
			...
			$options['enable_hierarchy'] = get_option( 'woostify_smart_product_filter_enable_hierarchy', '1' );
							...
							<div class="woostify-settings-section-content woostify-settings-section-tab">
								...
								<div class="woostify-setting-tab-content-wrapper">
									...
									<table class="form-table woostify-setting-tab-content" data-tab="general">
										...
										<tr>
											<th scope="row"><?php esc_html_e( 'Hierarchy', 'woostify-pro' ); ?>:</th>
											<?php 
												// When Enable Hierarchy is ON
												// Just only display attributes filter are include in cate filter
												// Example: Cate A,B has M, S, L size
												// when filter A just only display M, S are in include cate A
											?>
											<td>
												<input name="woostify_smart_product_filter_enable_hierarchy" type="checkbox" <?php checked( $options['enable_hierarchy'], '1' ); ?> value="<?php echo isset( $options['enable_hierarchy'] ) ? esc_attr( $options['enable_hierarchy'] ) : '1'; ?>">
												<span class="woostify-filter-enable-hierarchy"><?php esc_html_e( 'Enable hierarchy', 'woostify-pro' ); ?></span>
											</td>
										</tr>
										<tr>
							...
							<div class="w-filter-container">
								<div class="w-filter-field">
									<select name="woostify_product_filter_data" class="<?php echo esc_attr( $class_condition ); ?>">
										...
										<?php
										$product_attr_name_arr = array();
										if ( ! empty( $product_attr ) ) {
											foreach ( $product_attr as $tax ) {
												if ( taxonomy_exists( wc_attribute_taxonomy_name( $tax->attribute_name ) ) ) {
													array_push( $product_attr_name_arr, wc_attribute_taxonomy_name( $tax->attribute_name ) );
													?>
										...
										$taxonomys = get_object_taxonomies( array(
											'product',
										) );

										$taxonomy_no_display = array(
											'product_cat',
											'product_tag',
											'product_type',
											'product_visibility',
										...
										$taxonomy_no_display = array_merge($product_attr_name_arr, $taxonomy_no_display);
								
										if ( !empty( $taxonomys ) ) {
											foreach ($taxonomys as $key => $tax) {
												$taxonomy = get_taxonomy($tax);
												if ( taxonomy_exists( $tax ) ) {
													if( !in_array($tax, $taxonomy_no_display) ){
														?>
														<option class ="product-taxonomy <?php echo esc_attr( $tax ); ?>" value="<?php echo esc_attr( $tax ); ?>" <?php selected( $filter_data, $tax ); ?>>
															<?php esc_html_e( $taxonomy->label, 'woostify-pro' ); ?>
														</option>
														<?php
													}
												}
		...
		public function add_filter_key_remove() {
			if ( class_exists( 'Woostify_Woo_Builder' ) ) {
				...
				if ( 
					$woo_builder->product_page_woobuilder( 'single' ) 
					// || $woo_builder->shop_archive_woobuilder() 
				) 
				{
					return;
				}
			...
			$woostify_display_history_filter = !empty(get_option( 'woostify_display_history_filter' )) ? get_option( 'woostify_display_history_filter' ) : 'active';
			$is_display_history_filter = !isset($woo_builder) || ($woo_builder && (!$woo_builder->shop_archive_woobuilder() || ($woo_builder->shop_archive_woobuilder() && $woostify_display_history_filter == 'active')));
			if ($is_display_history_filter) {
				$data = $this->filter_parse_args( $output, true );
				?>
				...
				<?php if (!empty($data['key'])) {
					...
					?>
						<span class="w-filter-key-remove" data-type="all"><?php esc_attr_e('Clear', 'woostify-pro' ) ?> 
						<span class="w-filter-key-remove-icon"><?php echo $remove_icon; ?></span></span>
						<?php echo $data['key']; ?>
				<?php } ?>
				</div>
			<?php }
		...
		public function add_column_content( $column_name, $post_id ) {
			...
			switch ( $column_name ) {
				...
				case 'product_filter_data':
					...
					if( in_array( $data, $taxonomys ) ){
						$taxonomy = get_taxonomy( $data );	
						$title = __( $taxonomy->label, 'woostify-pro' );
					}
		...
		public function enqueue_assets() {
			...
			$data = array(
				...
				'expand_icon'   => '[+]',
				'collapse_icon' => '[-]',
				'enable_hierarchy' => isset( $options['enable_hierarchy'] ) ? $options['enable_hierarchy'] : TRUE,
			);
		...
		public function get_db_info() {
			...
			global $wpdb;
			$table_name = $this->table_name();
			$sql        = "SELECT DISTINCT product_id FROM $table_name WHERE product_id <> 0 and variation_id=0";
		...
		/**
		 * Return paged for the query
		 */
		public function get_paged_from_request(){
			$paged = isset( $_POST['paged'] ) 
			? intval( wp_unslash( $_POST['paged'] ) ) 
			: ( isset( $_POST['product-page'] ) 
				? intval( wp_unslash( $_POST['product-page'] ) ) 
				: ( get_query_var( 'paged' ) ? get_query_var( 'paged' ) : ( isset( $_GET['product-page'] ) 
				    ? intval( wp_unslash( $_GET['product-page'] ) ) 
					: 1
				  ) 
				)
			);
		...
		public function woostify_product_filter() {
			...
			$paged        = $this->get_paged_from_request(); 
			$res['paged'] = $paged;
			...
			$remove_icon  = woostify_fetch_svg_icon( 'close' );
			$orderby      = ! empty( $_POST['orderby'] ) ? sanitize_text_field( wp_unslash( $_POST['orderby'] ) ) : $default_orderby;
			$order      = ! empty( $_POST['order'] ) ? sanitize_text_field( wp_unslash( $_POST['order'] ) ) : 'ASC';
			$exclude_products = ! empty( $_POST['exclude_products'] ) ? json_decode(sanitize_text_field( wp_unslash( $_POST['exclude_products'] ) ) ) : '';
			...
			$outofstock_to_bottom = ( isset( $options['outofstock_to_bottom'] ) && ( $options['outofstock_to_bottom'] == 1 ) ); // Bool
			if ( 'yes' !== $hide_outstock  && $outofstock_to_bottom) {
				$args['orderby'] = array( 
					'stock_clause' => 'ASC',
				);
				$args['meta_query'] = array(
			...
			$order_items =  array('rating', 'popularity', 'date', 'price', 'price-desc');
			if ( in_array($orderby, $order_items )) {
				...
				if (isset($args['orderby']['order_clause'])) {
					$args['orderby']['order_clause'] = 'DESC';
				}

				switch ( $orderby ) {
					case 'rating':
						$args['meta_query']['order_clause']['key'] = '_wc_average_rating'; // phpcs:ignore
						break;

					case 'popularity':
						$args['meta_query']['order_clause']['key'] = 'total_sales'; // phpcs:ignore
						break;

					case 'date':
						unset( $args['meta_query']['order_clause'] );
						if (isset($args['orderby']['order_clause'])) {
							unset( $args['orderby']['order_clause'] );
						}
						if (isset($args['orderby']['post_date'])) {
					...
					case 'price':
						$args['meta_query']['order_clause']['key'] = '_price'; // phpcs:ignore
						if (isset($args['orderby']['order_clause'])) {
							$args['orderby']['order_clause'] = 'ASC';
						}
						break;
			...
			if (!empty($exclude_products)) {
				$exclude_products_ids = array_merge($exclude_products_ids,$exclude_products);
				$args['post__not_in'] = $exclude_products_ids;
			}
			...
			if ( isset( $parse_args['attr_args'] ) ) {
				...
				if ( ! empty( $outstock_variation ) && 'yes' === $hide_outstock ) {
					if ( empty( $args['post__in'] ) ) {
						// $args['post__not_in'] = $outstock_variation;
						$args['post__not_in'] = $exclude_products_ids;
					} else {
						// $args['post__in'] = array_diff( $args['post__in'], $outstock_variation );
						$args['post__in'] = array_diff( $args['post__in'], $exclude_products_ids );
					}
				}
			}
			...
			$products->set('__woostify_custom', array(
				'is_wooostify_filter' => true
			) );
			$wp_query = $products; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
			if ( ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ) {
				$wp_query->have_post();
				$res['sql'] = $wp_query->request;
			}
			...
			if ( $products->have_posts() ) {
				...
				$base = wp_get_referer();
				$pattern = '/\/page\/\d+/';
    			$base = preg_replace($pattern, '/page/%#%', $base, -1, $count1);
				$pattern = '/(product-){0,1}page=\d+/';
    			$base = preg_replace($pattern, 'page=%#%', $base, -1, $count2);
				if( empty( $count1 ) && empty( $count2 ) ) {
					$add = '/page/%#%/';
					$parsed_url = parse_url($base);
					$new_url = $parsed_url['scheme'] . '://' . $parsed_url['host'] . untrailingslashit( $parsed_url['path'] ) . $add;
					...
					// $base = add_query_arg( 'product-page', '%#%', $base );
				}
				$format = '';
				ob_start();
				...
				// Order loop
				// Refrence function woocommerce_catalog_ordering.
				ob_start();
		...
		public function get_sidebar() {
			...
			if ( ( 'horizontal' === $filter_options['layout'] && woostify_is_product_archive() && 'full' === $sidebar_shop ) || ( ! empty( $_POST['action'] ) && $_POST['action'] === 'woostify_product_filter' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.PHP.YodaConditions.NotYoda
				get_sidebar( 'shop' );
			}
		}
