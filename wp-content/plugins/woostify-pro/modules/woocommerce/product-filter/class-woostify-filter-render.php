...
if ( ! class_exists( 'Woostify_Filter_Render' ) ) {
	...
	class Woostify_Filter_Render {
		 ...
		 * @param array   $checked_value The selected value.
		 * @param array   $selected_value The current selected value.
		 * @param array   $is_tax The taxonomy page
		 * @param boolean $expand_default Expand by default.
		 */
		public function render_hierarchical( $filter_id, $output, $checked_value, $selected_value, $expand_default, $limit, $is_tax ) {
			...
			foreach ( $values as $key => $value ) {
				...
				$checked = in_array($slug, $checked_value) ? 'checked' : '';
				$checkbox_is_active = $is_tax && !empty($selected_value) && $selected_value == $slug ? TRUE : FALSE;
				?>
				<div class="w-filter-item-wrap <?php echo esc_attr( $checkbox_is_active ? 'w-filter-active woostify-theme-color' : '' ); ?>">
					<label class="w-filter-item" for="<?php echo esc_attr( $checkbox_html_id ); ?>" <?php echo esc_attr( $slug_attr ); ?> data-id="<?php echo esc_attr( $checkbox_id ); ?>">
						<?php if (!$checkbox_is_active) {?>
							<input class="w-filter-item-input" id="<?php echo esc_attr( $checkbox_html_id ); ?>" <?php echo $checked; ?> type="checkbox" name="w-filter-checkbox-<?php echo esc_attr( $value['term'] ); ?>">
						<?php } ?>
		 ...
		 */
		public function get_term_count( $filter_id, $term_id, $count, $list_products ) {
			$count = (int) $count;
			$type  = get_post_meta( $filter_id, 'woostify_product_filter_type', true );
			
			// Return when have condition filter by term
			$condition_filter_count = $this->get_condition_filter_term_count( $filter_id, $term_id, $list_products );

			if ( $condition_filter_count != -1 ) {
				$count = $condition_filter_count; 
				return $count;
			}
			...
			if (!empty($list_products['in'])) {
				$args['post__in'] = $list_products['in'];
			}

			if (!empty($list_products['notIn'])) {
				$args['post__not_in'] = $list_products['notIn'];
			}
		...
		/**
		 * Get count product when filter have condition by term
		 */
		public function get_condition_filter_term_count( $filter_id, $term_id, $list_products ) {
			global $wpdb;

			$count = -1;

			$woostify_new_products_exclude_product_ids = json_decode(get_option( 'woostify_new_products_exclude_product_ids' ));

			$filter_ids = $wpdb->get_col("SELECT ID
				FROM $wpdb->posts 
			...
			if ($woostify_new_products_exclude_product_ids) {
				$args['post__not_in'] = $woostify_new_products_exclude_product_ids;
			}
			...
			if ( $filter_id ) {
				...
				if ( $filter_ids ) {
					foreach ($filter_ids as $key => $id) {
						$condition_select = get_post_meta( $id, 'woostify_product_filter_term_condition_select', true );
						$condition_field  = get_post_meta( $id, 'woostify_product_filter_term_condition_field', true );
						$filter_data = get_post_meta( $id, 'woostify_product_filter_data', true );
						
						if($condition_select && $condition_field){
								
							$condition_field = explode( PHP_EOL, $condition_field );
							foreach ( $condition_field as $name ) {
								$condi_term = get_term_by( 'name', $name, $filter_data );
								if ( $condi_term ) {
							...
							if( !empty($term_arr) && taxonomy_exists( $filter_data )) {
								$args['tax_query'][] = array(
									'taxonomy' => $filter_data,
									'field'		 => 'term_id',
									'terms'    => $term_arr,
									'operator' => $condition,
								);
			...
			if ( 'yes' === $hide_outstock ) {
				...
				$outofstock_term          = $product_visibility_terms['outofstock'];
		...
		public function term_category_not_products($filter_id, $tax_name) {
			...
			$filter_data = get_post_meta( $filter_id, 'woostify_product_filter_data', true );
			
			$term_arr = array();
			$condition = '';
			if($condition_select && $condition_field){
				
				if( taxonomy_exists( $filter_data ) ){
					$condition = ( $condition_select == 'exclude' )? 'NOT IN' : 'IN';
					$condition_field = explode( PHP_EOL, $condition_field );
					foreach ( $condition_field as $name ) {
						$condi_term = get_term_by( 'name', $name, $filter_data );
		...
		public function render_filter( $filter_id = false, $product_ids = array(), $selected_value = false, $data = array(), $empty = false, $is_tax = false, $taxonomy = false ) {
			...
			$table_index_name = $wpdb->prefix . 'woostify_product_index';
			$options        = $product_filter->get_options();
			...
			$genaral_enable_hierarchy = $options['enable_hierarchy'];
			if ( $product_filter->detect_url_params() && ($selected_value || !empty($genaral_enable_hierarchy))) {
			...
			$condition_select    = get_post_meta( $filter_id, 'woostify_product_filter_term_condition_select', true );
			$condition_field     = get_post_meta( $filter_id, 'woostify_product_filter_term_condition_field', true );
			$product_filter_data = get_post_meta( $filter_id, 'woostify_product_filter_data', true );
			$limit               = (int) $product_filter->set_value( $filter_id, 'woostify_product_filter_limit', 0 );
			...
			// Filter ajax conflict with get_queried_object() : Do not use get_queried_object() to detect tax page
			if ( $is_tax || ( ! empty( get_queried_object() ) && isset( get_queried_object()->term_id ) ) ) {
				$checkterm = get_term( $product_taxonomy_id[0], $taxonomy );
				$checkcat = false;
				if ( ! $taxonomy ) {
					$checkcat = get_the_category_by_ID( $product_taxonomy_id[0] );
				}

				if ( ( ! $checkcat ) && $checkterm && $checkterm->count == 0 ) { //phpcs:ignore
					$empty = true;
				}

				if (!empty($checkterm)) {
					$selected_value = $checkterm->slug;
				}
			}
			...
			if ( ! empty( $condition_term ) ) {
				if ( empty( $product_taxonomy_id ) ) {
					...
					if ( isset( $condition_term['include'] ) ) {
						...
						if ( has_term( $selected_value, $source ) ) {
			...
			if ( 'yes' === $hide_outstock ) {
				...
				if ( ! empty( $outofstock_term ) ) {
					$where_term_ids .= " AND term_id NOT IN ($outofstock_term) ";
				}
			}
			...
			if ( empty( $product_taxonomy_id ) ) {
				if ( 'yes' === get_option( 'woocommerce_hide_out_of_stock_items' ) && ! empty( $outstock_ids ) ) {
					$list_products['notIn'] = $outstock_ids;
				...
				if ( ! empty( $all_ids_in_term ) ) {
					$list_products['in'] = $all_ids_in_term;
			...
			if ( empty( $product_ids ) ) {
				...
				if ( 'yes' === get_option( 'woocommerce_hide_out_of_stock_items' ) && ! empty( $outstock_ids ) ) {
					$list_products['in'] = array_diff( $product_ids, $outstock_ids );
					$where_product_ids = 'AND product_id IN (' . implode( ',', array_diff( $product_ids, $outstock_ids ) ) . ')';
				} else {
					$list_products['in'] = $product_ids;
			...
			if ($first_active_filter) {
				$w_product_ids = $all_ids_in_term;
			}

			$term_cat_not_products = $this->term_category_not_products( $filter_id, $source);
			
			// Counter.
			$item_count = $options['general_item_count'];
			
			$item_filter_empty = FALSE;
			// Filter content.
			ob_start();
			switch ( $type ) {
				case 'visual':
					...
					$sql         = "SELECT DISTINCT term_name, term_id, term FROM $table_name as tn ";
					$visual_where = "WHERE term='$swatch_slug' $where_product_ids $where_term_ids ";
					if ( defined( 'ICL_SITEPRESS_VERSION' ) && defined( 'ICL_LANGUAGE_CODE' ) ) {
						$sql .= "INNER JOIN $table_index_name as wpi ON wpi.id = tn.product_id ";
						$visual_where .= "AND wpi.lang= '" . ICL_LANGUAGE_CODE . "'";
					}
					$sql .= $visual_where;
					$output      = $wpdb->get_results($sql, ARRAY_A); // phpcs:ignore
					
					if ( empty( $output ) ) {
						$item_filter_empty = TRUE;
						break;
					}

					// Load data form url.
					$swatch_selected_value        = $get_url_params ? array_map( 'intval', explode( ',', $get_url_params ) ) : array();
					if ( !empty( $selected_value )) {
						array_push($swatch_selected_value, $selected_value);
					}
					...
					foreach ( $output as $pat ) {
						...
						switch ( $swatch_type ) {
							case 'color':
								// Load data form url.
								$color_selected = $get_url_params ? explode( ',', $get_url_params ) : array();
								// Checked value.
								if ( !empty( $selected_value )) {
									array_push($color_selected, $selected_value);
								}
								$selected_visual = in_array( $slug, $color_selected ) ? 'selected' : '';
								?>
				...
				case 'stock':
					...
					// Checked value.
					if (!empty( $selected_value )) {
						array_push($get_url_params, $selected_value);
					}
					...
					foreach ( $stock as $value ) {
						...
						$count            = $this->get_term_count( $filter_id, $value, 0, $list_products );
						if ( $count > 0 && isset( $stock_data[ $value ] ) ) {
							...
							<div class="w-filter-item-wrap">
								<label class="w-filter-item" for="<?php echo esc_attr( $checkbox_html_id ); ?>" data-slug="<?php echo esc_attr( $value ); ?>">
									<input class="w-filter-item-input" id="<?php echo esc_attr( $checkbox_html_id ); ?>" <?php echo in_array( $value, $get_url_params, true ) ? 'checked' : ''; ?> type="checkbox" name="w-filter-checkbox-<?php echo esc_attr( $source ); ?>">
				...
				case 'checkbox':
					...
					// Load data form url.
					$checked_value = $get_url_params ? explode( ',', $get_url_params ) : array();

					// Checked value.
					if (!empty( $selected_value )) {
						array_push($checked_value, $selected_value);
					}
					...
					$expand_default = get_post_meta( $filter_id, 'woostify_product_filter_term_hierarchical_expand', true );
					if( $hierarchical ) {
						$taxonomy = get_post_meta( $filter_id, 'woostify_product_filter_data', true );
					}
					$source = $hierarchical ? $taxonomy : $source;
					...
					$sql = "SELECT term_name, term_id, term, parent_id, depth, COUNT(DISTINCT tn.product_id) as count FROM $table_name as tn ";
					$cb_where = "WHERE term='$source' AND term <> '' AND term_id <> 0 $where_product_ids $where_term_ids ";
					if ( defined( 'ICL_SITEPRESS_VERSION' ) && defined( 'ICL_LANGUAGE_CODE' ) ) {
						$table_index_name = $wpdb->prefix . 'woostify_product_index';
						$sql .= "INNER JOIN $table_index_name as wpi ON wpi.id = tn.product_id ";
						$cb_where .= "AND wpi.lang= '" . ICL_LANGUAGE_CODE . "' ";
					}
					$sql .= $cb_where . "GROUP BY term_id ORDER BY depth, $orderby";
					$output = $wpdb->get_results($sql, ARRAY_A); // phpcs:ignore
					...
					if ( $hierarchical ) {
						$this->render_hierarchical( $filter_id, $output, $checked_value, $selected_value, $expand_default, $limit, $is_tax );
						break;
					}
					...
					foreach ( $output as $key => $check ) { 
						...
						$checkbox_count   = $this->get_term_count( $filter_id, $checkbox_id, $check['count'], $list_products );
						...
						if ( $checkbox_count > 0 && !in_array( $checkbox_id, $term_cat_not_products ) ) {
							$checkbox_is_active = $is_tax && !empty($selected_value) && $selected_value == $slug ? TRUE : FALSE;
						?>
						<div class="w-filter-item-wrap <?php echo esc_attr( $checkbox_is_active ? 'w-filter-active woostify-theme-color' : '' ); ?>">
							<label class="w-filter-item" for="<?php echo esc_attr( $checkbox_html_id ); ?>" <?php echo esc_attr( $slug_attr ); ?> data-id="<?php echo esc_attr( $checkbox_id ); ?>">
								<?php if (!$checkbox_is_active) {?>
									<input class="w-filter-item-input" id="<?php echo esc_attr( $checkbox_html_id ); ?>" <?php echo (!empty($get_url_params) && in_array( $slug, $checked_value)) ? 'checked' : ''; ?>  type="checkbox" name="w-filter-checkbox-<?php echo esc_attr( $source ); ?>">
								<?php } ?>
				...
				case 'radio':
					...
					$sql = "SELECT DISTINCT term_name, term, term_id, COUNT(DISTINCT tn.product_id) as count FROM $table_name as tn ";
					$radio_where = "WHERE term='$source' AND term <> '' AND term_id <> 0 $where_product_ids $where_term_ids ";
					if ( defined( 'ICL_SITEPRESS_VERSION' ) && defined( 'ICL_LANGUAGE_CODE' ) ) {
						$table_index_name = $wpdb->prefix . 'woostify_product_index';
						$sql .= "INNER JOIN $table_index_name as wpi ON wpi.id = tn.product_id ";
						$radio_where .= "AND wpi.lang= '" . ICL_LANGUAGE_CODE . "' ";
					}
					$sql .= $radio_where . "GROUP BY term_id ORDER BY $orderby";
					$output               = $wpdb->get_results( $sql, ARRAY_A ); // phpcs:ignore
					...
					foreach ( $output as $key => $radio ) {
						...
						$radio_count   = $this->get_term_count( $filter_id, $radio_id, $radio['count'], $list_products );
						...
						if ($radio_count > 0 && !in_array( $radio_id, $term_cat_not_products )) {
							
						$radio_is_active = $is_tax && !empty($selected_value) && $selected_value == $slug ? TRUE : FALSE;
						?>
						<div class="w-filter-item-wrap <?php echo esc_attr( $radio_is_active ? 'w-filter-active woostify-theme-color' : '' ); ?>">
							<label class="w-filter-item" for="<?php echo esc_attr( $radio_html_id ); ?>" <?php echo esc_attr( $slug_attr ); ?> data-id="<?php echo esc_attr( $radio_id ); ?>">
								<?php if (!$radio_is_active) { ?>
									<input class="w-filter-item-input" id="<?php echo esc_attr( $radio_html_id ); ?>" <?php checked( $get_url_params, $slug ); ?> type="radio" name="w-filter-radio-<?php echo esc_attr( $source ); ?>">
								<?php } ?>
				...
				case 'select':
					...
					$sql    = "SELECT DISTINCT term_name, term, term_id, COUNT(DISTINCT tn.product_id) as count FROM $table_name as tn ";
					$select_where = "WHERE term='$source' AND term <> '' AND term_id <> 0 $where_product_ids $where_term_ids ";
					if ( defined( 'ICL_SITEPRESS_VERSION' ) && defined( 'ICL_LANGUAGE_CODE' ) ) {
						$sql .= "INNER JOIN $table_index_name as wpi ON wpi.id = tn.product_id ";
						$select_where .= "AND wpi.lang= '" . ICL_LANGUAGE_CODE . "' ";
					}
					$sql .= $select_where . "GROUP BY term_id ORDER BY $orderby";
					$output = $wpdb->get_results($sql, ARRAY_A); // phpcs:ignore
					...
					foreach ( $output as $key => $select ) {
						$select_count = $this->get_term_count( $filter_id, $select['term_id'], $select['count'] , $list_products );
						...
						if ( $select_count > 0 && !in_array( $select['term_id'], $term_cat_not_products ) ) {
							?>
							<option value="<?php echo esc_attr( $slug ); ?>" <?php selected( $_selected_value, $slug, true ); ?> >
							<?php
							if ( $item_count ) {
				...
				case 'range-slider':
					...
					$min_max = "SELECT min(tn.price) as min_price, max(tn.price) as max_price FROM $table_name as tn ";
					$rangle_where = "WHERE tn.price>=0 $where_product_ids ";
					if ( defined( 'ICL_SITEPRESS_VERSION' ) && defined( 'ICL_LANGUAGE_CODE' ) ) {
						$min_max .= "INNER JOIN $table_index_name as wpi ON wpi.id = tn.product_id ";
						$rangle_where .= "AND wpi.lang= '" . ICL_LANGUAGE_CODE . "'";
					}
					$min_max .= $rangle_where;
					$min_max = $wpdb->get_results( $min_max, ARRAY_A ); // phpcs:ignore

					$min = isset( $min_max[0]['min_price'] ) ? $min_max[0]['min_price'] : 0;
					$max = isset( $min_max[0]['max_price'] ) ? $min_max[0]['max_price'] : 0;
					if ( $min === $max ) {
						break;
					}
					...
					} elseif ( ! empty( $selected_value ) && is_numeric($selected_value[0]) && is_numeric($selected_value[1])) {
					...
					<button style="display: <?php echo $slider_value ? 'block' : 'none' ?>" class="w-filter-range-slider-reset w-filter-item-submit"><?php esc_html_e( 'Reset', 'woostify-pro' ); ?></button>
					<?php
					break;
				case 'check-range':
					if ( $empty ) {
					...
					// $checked_range = empty( $selected_range ) ? $checked_range_url : $selected_range;
					$checked_range = $checked_range_url;
				...
				case 'date-range':
					...
					// Load data from url.
					$picked_date = $get_url_params ? explode( ',', $get_url_params ) : array();
					// if ( !empty( $selected_value )) {
					// 	array_push($picked_date, $selected_value);
					// }
				...
				case 'search':
					...
					$searched_value = $get_url_params ? $get_url_params : '';
					?>
			...
			if ($item_filter_empty) { ?>
				<span class="w-product-filter"></span>
			<?php } else if ( ! empty( $filter_content ) ) {
				?>
				<div class="w-product-filter <?php echo esc_attr( 'w-product-filter-type-' . $type ); ?> <?php echo isset( $options['collapse_enabled'] ) && '1' === $options['collapse_enabled'] ? '' : 'no-collapse'; ?>" data-type="<?php echo esc_attr( $type ); ?>" data-id="<?php echo esc_attr( $filter_id ); ?>">
					<?php
					// Filter title.
					if ( ! empty( $label ) ) {
						?>
		...
		public function render_all_filter( $attrs = array() ) {
			...
			$is_tax = FALSE;
			$taxonomy = FALSE;
			if ($wp_query->is_tax && $wp_query->get_queried_object()->term_id) {
				$is_tax = $wp_query->get_queried_object()->term_id;
			...
			<div class="w-product-filter-all">
				<?php
				foreach ( $query as $id ) {
					$this->render_filter( $id, array(), false, array(), false, $is_tax, $taxonomy);
				}
				?>
			</div>
			<?php
