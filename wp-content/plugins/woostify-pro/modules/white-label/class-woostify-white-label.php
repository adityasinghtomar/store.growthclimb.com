	...
	class Woostify_White_Label {
		...
		public function hook() {
			...
			if ( is_admin() ) {
				...
				add_filter( 'plugin_row_meta', array( $this, 'woostify_white_label_plugin_row_meta' ), 10, 4 );
		 ...
		 * White labels the plugins page.
		 *
		 * @param array $plugin_meta Plugins Links Array.
		 * @param string $plugin_file Plugins file.
		 * @param array $plugin_data Plugins Data Array.
		 * @param string $status status
		 * @return array $plugin_meta New Plugins Links Array.
		 */
		public function woostify_white_label_plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
			if ( plugin_basename( WOOSTIFY_PRO_PATH . 'woostify-pro.php' ) === $plugin_file ) {
				$options = $this->get_options();
				$author_uri = $options['woostify_wl_agency_author_url'];
				$label = __('Visit plugin site', 'woostify-pro');
				$name = $options['woostify_wl_plugin_name'];
				$newLink = '<a href="' . esc_url($author_uri) . '" aria-label="' . esc_attr($name) . '" data-title="' . esc_attr($name) . '">' . $label . '</a>';

				if( is_array( $plugin_meta )){
					foreach($plugin_meta as $i => $link){
						if( strpos( $link, 'wp-admin/plugin-install.php') !== false ){
							$plugin_meta[$i] = $newLink;
						}
					}
				}else{
					$plugin_meta = array($newLink);
				}
			}
			if ( 'woostify-sites-library/woostify-sites.php' === $plugin_file ) {
				$options = $this->get_options();
				$author_uri = $options['woostify_wl_agency_author_url'];
				$label = __('Visit plugin site', 'woostify-pro');
				$name = $options['woostify_wl_library_plugin_name'];
				$newLink = '<a href="' . esc_url($author_uri) . '" aria-label="' . esc_attr($name) . '" data-title="' . esc_attr($name) . '">' . $label . '</a>';
