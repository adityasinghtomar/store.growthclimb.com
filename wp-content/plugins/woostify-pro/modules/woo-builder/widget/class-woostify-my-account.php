...
class Woostify_My_Account extends Widget_Base {
	...
	public function get_script_depends() {
		return array( 'woostify-my-account-widget', 'woostify-infinite-scroll-plugin' );
	}
						...
						switch ( $nav_item ) {
							...
							case 'orders':
								if ( $is_view_order ) {
									...
									global $wp;
									$order_base = str_replace( add_query_arg( array(), $wp->request ), "", add_query_arg( $wp->query_vars ));
									$find_order_postion = strpos( $order_base, 'orders' );
									$order_substr = substr( $order_base, $find_order_postion, strlen($order_base) );
									$order_explode = explode( "=", $order_substr );
									$current_page = '';
									if ( count($order_explode) > 1 && $order_explode[1] ) {
										$current_page = $order_explode[1];
									}
									do_action( 'woocommerce_account_' . $nav_item . '_endpoint', $current_page );
									...
									$total_page = ceil($total_records / $posts_per_page);
									...
									if ( $options['shop_page_infinite_scroll_enable'] && $total_page > 1 ) {
										$infinite_scroll_type = $options['shop_page_infinite_scroll_type'];
										?>
										<div class="woostify-view-more-order" 
											data-loading_type="<?php echo $infinite_scroll_type; ?>" 
											data-total_page="<?php echo $total_page; ?>">
											<?php if( $infinite_scroll_type == 'button' ): ?>
											<button class="w-view-more-button products-archive button">
												<span class="w-view-more-label"><?php esc_html_e('View more','woostify-pro'); ?></span>
											</button>
											<?php else: ?>
										...
										echo '<div class="order-pagination woocommerce-pagination">';
...
Plugin::instance()->widgets_manager->register( new Woostify_My_Account() );
