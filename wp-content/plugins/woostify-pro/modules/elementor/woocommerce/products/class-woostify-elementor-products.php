...
class Woostify_Elementor_Products extends Woostify_Elementor_Products_Base {
	...
	protected function render() {
		...
		if ( class_exists( 'Woostify_Product_Filter' ) && 'yes' === $settings['show_remove_filter'] && woostify_is_elementor_editor()) { ?>
		...
		$exclude_product_ids = json_encode($settings['exclude_products']);
		if ( get_option( 'woostify_new_products_exclude_product_ids' ) ) {
			update_option( 'woostify_new_products_exclude_product_ids',  $exclude_product_ids );
		}else{
			add_option( 'woostify_new_products_exclude_product_ids', $exclude_product_ids );
		}

		$show_filter = (isset($settings['show_remove_filter']) && $settings['show_remove_filter'] == 'yes' ) ? 'active' : 'none';

		if ( get_option( 'woostify_display_history_filter' ) ) {
			update_option( 'woostify_display_history_filter',  $show_filter );	
		} else {
			add_option( 'woostify_display_history_filter', $show_filter );
		}
		?>
		<div class="woostify-products-widget w-result-filter"
			data-posts="<?php echo $settings['count']; ?>"
			data-posts-orderby="<?php echo $settings['order_by']; ?>"
			data-posts-order="<?php echo $settings['order']; ?>"
			data-exclude-products="<?php echo $exclude_product_ids;?>">
			<?php echo $content; // phpcs:ignore ?>
		</div>
		<?php
	}
}
Plugin::instance()->widgets_manager->register( new Woostify_Elementor_Products() );
