...
if ( ! class_exists( 'Woostify_Pro' ) ) {
	...
	class Woostify_Pro {
		...
		public function woostify_pro_dashboard_static() {
			...
			$is_fbt                  = false !== strpos( $screen->id, 'frequently-bought-together' );
			...
			if ( $screen->post_type || $is_countdown_urgency || $is_swatches || $is_sale_notification || $is_ajax_search_product || $is_smart_product_filter || $is_welcome || $is_pre_order || $is_callback || $is_white_label || $is_fbt ) {
				wp_enqueue_script(
		...
		public function woostify_pro_modules() {
			...
			$modules = array(
				...
				'woostify_wc_callback'                   => array(
					...
					'category'    => array( 'ecommerce' ),
					'doc'         => esc_url( $woostify_url ) . '/docs/pro-modules/callback/',
				...
				'woostify_wc_pre_order'                  => array(
					...
					'category'    => array( 'ecommerce' ),
					'doc'         => esc_url( $woostify_url ) . '/docs/pro-modules/pre-order/',
				...
				'woostify_wc_frequently_bought_together' => array(
					'title'       => __( 'Frequently Bought Together', 'woostify-pro' ),
					'description' => __( 'Offers a way to get related products together and put them in a deal', 'woostify-pro' ),
					'icon'        => WOOSTIFY_PRO_URI . 'assets/images/module-icon-bought-together.png',
					'category'    => array( 'ecommerce' ),
					'doc'         => esc_url( $woostify_url ) . '/docs/pro-modules/frequently-bought-together/',
					'setting_url' => get_admin_url() . 'admin.php?page=frequently-bought-together',
					'condition'   => $woocommerce_condition,
					'error'       => $woocommerce_error,
				),
			);
		...
		public function module_list() {
			...
			if ( woostify_is_woocommerce_activated() ) {
				...
				if ( $this->module_active( 'woostify_wc_frequently_bought_together', 'WOOSTIFY_PRO_FREQUENT_BOUGHT_TOGETHER' ) ) {
					require_once WOOSTIFY_PRO_MODULES_PATH . 'woocommerce/frequently-bought-together/class-woostify-fbt.php';
				}
