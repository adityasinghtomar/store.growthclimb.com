		...
		public function save_options() {
			...
			if ( ! empty( $options ) ) {
				...
				foreach ( $options as $k => $v ) {
					...
					} elseif ( preg_match( '/(\w{1,})\[(\w{1,})\]/', $k, $matches ) ) {
						// Chỗ này code chữa cháy. Tốt hơn nên dùng formData để submit dữ liệu.
						$m_array[ $matches[1] ][ $matches[2] ] = $value;
					} else {
						update_option( $k, $value );
					}
				}
				if ( ! empty( $m_array ) ) {
					foreach ( $m_array as $k => $array ) {
