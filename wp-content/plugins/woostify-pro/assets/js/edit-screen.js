...
var woostifySaveOptions = function () {
	...
	var _id = box.getAttribute('data-id') || '',
		...
		save_option_messages = box.querySelector('.woostify-save-message'),
		...
		function (button) {
			button.onclick = function () {
				...
				if (options.length) {
					options.forEach(
						function( el, index ) {
							if ( ! el.value.trim() && ! el.classList.contains( 'woostify-admin-color-picker' ) && ! el.classList.contains( 'field-empty' ) && ( el.closest('.woostify-custom-field-control') == null ) ) {
								return;
							}
				...
				var request = new Request(
					...
					{
						method: 'POST',
						body: encodeURI('action=woostify_save_' + likeId + '_options&security_nonce=' + _nonce + '&setting_id=' + _id + '&options=' + JSON.stringify(value)),
						credentials: 'same-origin',
						headers: new Headers(
							{
								'Content-Type': 'application/x-www-form-urlencoded; charset=utf-8'
				...
				fetch(request)
					.then(
						function (res) {
							...
							// Show success mesages.
							if ( save_option_messages ) {
								save_option_messages.classList.add( 'success' );
								save_option_messages.querySelector('.message-success').innerHTML = woostify_edit_screen.saved_success;
								setTimeout(function () {
									save_option_messages.classList.remove( 'success' );
								},3000);
							}else{
...
// Close messages success save options.
var woostifySaveOptionMessageClose = function () {
	var message_close_button = document.querySelector('.woostify-save-message-close');

	if ( ! message_close_button ) {
		return;
	}

	message_close_button.addEventListener(
		'click',
		function (e) { 
