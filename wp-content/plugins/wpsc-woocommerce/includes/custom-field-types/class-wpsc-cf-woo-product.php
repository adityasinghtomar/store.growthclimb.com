...
if ( ! class_exists( 'WPSC_CF_WOO_Product' ) ) :

	final class WPSC_CF_WOO_Product {
		 ...
		 * @var string
		 */
		public static $slug = 'cf_woo_product';
		 ...
		 * @var string
		 */
		public static $ref_class = 'wpsc_woo_product';
		...
		public static function init() {
			...
			add_action( 'wp_ajax_wpsc_woo_product_autocomplete', array( __CLASS__, 'product_autocomplete' ) );
			add_action( 'wp_ajax_nopriv_wpsc_woo_product_autocomplete', array( __CLASS__, 'product_autocomplete' ) );
		}

		/**
		 * Load current class to ref classes
		 *
		 * @param array $classes - ref class filter.
		 * @return array
		 */
		 ...
		 *
		 * @param array $cf_types - custom field type filter array.
		 * @return array
		 */
		public static function add_cf_type( $cf_types ) {

			$cf_types[ self::$slug ] = array(
				'label' => esc_attr__( 'WOO Product', 'wpsc-woo' ),
				'class' => __CLASS__,
			);
			return $cf_types;
		}
		 ...
		 * @param WPSC_Custom_field $cf - custom field object.
		 * @param array             $filter - user filter (if any).
		 */
		public static function get_operands( $operator, $cf, $filter = array() ) {

			$is_multiple = $operator !== '=' ? true : false;
			$products    = array();
			if ( isset( $filter['operand_val_1'] ) && $is_multiple ) {

				foreach ( $filter['operand_val_1'] as $key => $id ) {
					$product = wc_get_product( intval( $id ) );
					if ( $product ) {
						$products[] = $product;
					}
				}
			} elseif ( isset( $filter['operand_val_1'] ) && ! $is_multiple ) {

				$product = wc_get_product( intval( $filter['operand_val_1'] ) );
				if ( $product ) {
					$products[] = $product;
				}
			}
			...
			<div class="item conditional operand single">
				<select class="operand_val_1 <?php echo esc_attr( $unique_id ); ?>" <?php echo $is_multiple ? 'multiple' : ''; ?>>
					...
					foreach ( $products as $product ) {
						?>
						<option value="<?php echo esc_attr( $product->get_id() ); ?>" selected><?php echo esc_attr( $product->get_name() ); ?></option>
						<?php
					}
					?>
				</select>
						...
						ajax: {
							...
							data: function (params) {
								return {
									...
									page: params.page,
									action: 'wpsc_woo_product_autocomplete',
									_ajax_nonce: '<?php echo esc_attr( wp_create_nonce( 'wpsc_woo_product_autocomplete' ) ); ?>'
								};
							},
		 ...
		 *
		 * @param array             $condition - condition to verify.
		 * @param WPSC_Custom_field $cf - custom field object.
		...
		public static function is_valid( $condition, $cf, $val ) {
			...
			switch ( $condition['operator'] ) {
				...
				case 'IN':
					$response = in_array( $val, $condition['operand_val_1'] ) ? true : false;
					break;
		...
		public static function is_valid_ticket_condition( $condition, $cf, $ticket ) {
			...
			switch ( $condition['operator'] ) {
				...
				case '=':
					$flag = $ticket->{$cf->slug}->get_id() == $condition['operand_val_1'] ? true : false;
					break;

				case 'IN':
					$flag = in_array( $ticket->{$cf->slug}->get_id(), $condition['operand_val_1'] );
					break;
		...
		public static function is_valid_customer_condition( $condition, $cf, $customer ) {
			...
			switch ( $condition['operator'] ) {
				...
				case '=':
					$flag = $customer->{$cf->slug}->get_id() == $condition['operand_val_1'] ? true : false;
					break;

				case 'IN':
					$flag = in_array( $customer->{$cf->slug}->get_id(), $condition['operand_val_1'] );
					break;
		...
		public static function print_tff( $cf, $tff ) {
			...
			$val = 0;
			if ( $cf->field == 'ticket' ) {
				$val = $cf->is_auto_fill && $cf->default_value ? wc_get_product( intval( $cf->default_value[0] ) ) : 0;
			} else {
				$current_user = WPSC_Current_User::$current_user;
				$val          = $current_user->is_customer && $current_user->customer->{$cf->slug} ? $current_user->customer->{$cf->slug} : 0;
			}

			// ignore auto-fill value if ticket is baing created for order help button.
			if ( $cf->field == 'ticket' && isset( $_POST['woo-product'] ) ) { // phpcs:ignore
				$product = wc_get_product( intval( $_POST['woo-product'] ) ); // phpcs:ignore
				$val     = $product ? $product : 0;
			}

			// auto-fill product if help tab option is enabled.
			$temp_pid = get_transient( 'wpsc_temp_product_id' );
			if ( $temp_pid ) {
				$val = wc_get_product( intval( $temp_pid ) );
				delete_transient( 'wpsc_temp_product_id' );
			}

			$woo_product = is_object( $val ) ? $val : false;
			$unique_id   = uniqid( 'wpsc_' );
			...
			<div class="<?php echo esc_attr( WPSC_Functions::get_tff_classes( $cf, $tff ) ); ?>" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<select class="<?php echo esc_attr( $unique_id ); ?>" name="<?php echo esc_attr( $cf->slug ); ?>" onchange="wpsc_check_tff_visibility()">
					...
					if ( $woo_product ) {
						?>
						<option value="<?php echo esc_attr( $woo_product->get_id() ); ?>"><?php echo esc_attr( $woo_product->get_name() ); ?></option>
						<?php
					}
					?>
				</select>
		 ...
		 *
		 * @param array             $data - create ticket data to filter.
		 * @param WPSC_Custom_Field $custom_fields - custom field object.
		 * @param boolean           $is_my_profile - set whether this function is being called from my profile.
		 * @return array
		 */
			...
			if ( isset( $custom_fields[ self::$slug ] ) ) {
				foreach ( $custom_fields[ self::$slug ] as $cf ) {
					...
					if ( $cf->field == 'ticket' ) {
						...
						$data[ $cf->slug ] = $cf->default_value ? $cf->default_value[0] : '';
			...
			if ( isset( $custom_fields[ self::$slug ] ) ) {
				foreach ( $custom_fields[ self::$slug ] as $cf ) {
					...
					if ( in_array( $cf->field, array( 'ticket', 'agentonly' ) ) ) {
						...
						$data[ $cf->slug ] = $cf->field == 'ticket' && $value && $product->get_id() == $value ? $value : '';
						...
						$value = $value && $product->get_id() == $value ? $value : '';
			...
			if ( isset( $custom_fields[ self::$slug ] ) ) {
				...
				foreach ( $custom_fields[ self::$slug ] as $cf ) {
					if ( in_array( $cf->slug, $allowed_search_fields ) ) {
						if ( ! $search_items ) {
							$search_items = self::get_woo_tl_search_items( $search );
							if ( ! $search_items ) {
								break;
							}
						}
		 ...
		 * @return array
		 */
		public static function get_woo_tl_search_items( $s ) {
		 ...
		 *
		 * @param WPSC_Custom_Field $cf - custom field object.
		 * @param mixed             $product - optional. Input field will be printed with given value.
		 * @return void
		 */
		public static function print_cf_input( $cf, $product = '' ) {
			...
			<div class="wpsc-tff wpsc-xs-12 wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<select class="<?php echo esc_attr( $unique_id ); ?>" name="<?php echo esc_attr( $cf->slug ); ?>">
					...
					if ( $product ) {
						?>
						<option selected value="<?php echo esc_attr( $product->get_id() ); ?>"><?php echo esc_attr( $product->get_name() ); ?></option>
						<?php
					}
					?>
				</select>
		...
		public static function print_edit_ticket_cf( $cf, $ticket ) {
			...
			<div class="wpsc-tff wpsc-xs-12 wpsc-sm-12 wpsc-md-12 wpsc-lg-12 wpsc-visible" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<select class="<?php echo esc_attr( $unique_id ); ?>" name="<?php echo esc_attr( $cf->slug ); ?>">
					<?php
					if ( $ticket->{$cf->slug} ) {
						$product = wc_get_product( esc_attr( $ticket->{$cf->slug}->get_id() ) );
						?>
						<option value="<?php echo esc_attr( $product->get_id() ); ?>"><?php echo esc_attr( $product->get_name() ); ?></option>
						<?php
					}
					?>
				</select>
		...
		public static function set_rest_edit_ticket_cf( $ticket, $cf, $value ) {
			...
			$prev = is_object( $ticket->{$cf->slug} ) ? $ticket->{$cf->slug} : '';
			if ( $prev != $value ) {
		...
		public static function print_edit_customer_info( $cf, $customer, $tff ) {
			...
			<div class="<?php echo esc_attr( WPSC_Functions::get_tff_classes( $cf, $tff ) ); ?>" data-cft="<?php echo esc_attr( self::$slug ); ?>">
				...
				<select class="<?php echo esc_attr( $unique_id ); ?>" name="<?php echo esc_attr( $cf->slug ); ?>">
					<?php
					if ( $customer->{$cf->slug} ) {
						$product = wc_get_product( esc_attr( $customer->{$cf->slug}->get_id() ) );
						?>
						<option value="<?php echo esc_attr( $product->get_id() ); ?>"><?php echo esc_attr( $product->get_name() ); ?></option>
						<?php
					}
		 ...
		 * @return void
		 */
		public static function product_autocomplete() {

			if ( check_ajax_referer( 'wpsc_woo_product_autocomplete', '_ajax_nonce', false ) !== 1 ) {
		 ...
		 */
		public static function get_tl_search_items( $s ) {
		...
		public static function get_add_new_custom_field_properties( $field_class ) {
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
				...
				<div data-type="single-select" data-required="false" class="wpsc-input-group default_value">
					<div class="label-container">
						<label for="">
							<?php echo esc_attr( wpsc__( 'Default value', 'supportcandy' ) ); ?>
						</label>
					</div>
					<div class="ticket-fields-assigned-agent">
						<select class="wpsc-woo-product-default-val" name="default_value"></select>
					</div>
					<script>
						jQuery('.wpsc-woo-product-default-val').selectWoo({
							ajax: {
								url: supportcandy.ajax_url,
								dataType: 'json',
								delay: 250,
			...
			if ( in_array( 'is_auto_fill', $field_class::$allowed_properties ) ) :
				...
				<div data-type="single-select" data-required="false" class="wpsc-input-group is_auto_fill">
					<div class="label-container">
						<label for="">
							<?php echo esc_attr( wpsc__( 'Auto-fill in ticket form', 'supportcandy' ) ); ?>
						</label>
					</div>
		...
		public static function get_edit_custom_field_properties( $cf, $field_class ) {
			...
			if ( in_array( 'default_value', $field_class::$allowed_properties ) ) :
				$product = $cf->default_value ? wc_get_product( $cf->default_value[0] ) : false
				?>
				<div data-type="single-select" data-required="false" class="wpsc-input-group default_value">
					<div class="label-container">
						<label for="">
					...
					<div class="ticket-fields-assigned-agent">
						<select class="wpsc-woo-product-default-val" name="default_value">
						<?php
						if ( $product ) {
							?>
							<option value="<?php echo esc_attr( $product->get_id() ); ?>"><?php echo esc_attr( $product->get_name() ); ?></option>
			...
			if ( in_array( 'is_auto_fill', $field_class::$allowed_properties ) ) :
				...
				<div data-type="single-select" data-required="false" class="wpsc-input-group is_auto_fill">
					...
					<select name="is_auto_fill">
						<option <?php selected( $cf->is_auto_fill, '0' ); ?> value="0"><?php echo esc_attr( wpsc__( 'No', 'supportcandy' ) ); ?></option>
						<option <?php selected( $cf->is_auto_fill, '1' ); ?> value="1"><?php echo esc_attr( wpsc__( 'Yes', 'supportcandy' ) ); ?></option>
					</select>
				</div>
				<?php
			endif;
		...
		public static function get_ticket_field_val( $cf, $ticket, $module = '' ) {
			...
			$product = $ticket->{$cf->slug};
			$value = $product ? ( $product->get_name() ) : '';
			return apply_filters( 'wpsc_ticket_field_val_woo_product', $value, $cf, $ticket, $module );
		}
		...
		public static function print_widget_ticket_field_val( $cf, $ticket ) {
			...
			?>
			<a class="wpsc-link" href="<?php echo esc_url_raw( get_permalink( $product->get_id() ) ); ?>" target="_blank"><?php echo esc_html( $product->get_name() ); ?></a>
			<?php
		}
		...
		public static function get_customer_field_val( $cf, $customer ) {
			...
			$product = $customer->{$cf->slug};
			return $product ? get_the_title( $product->get_id() ) : '';
		}
		...
		public static function print_val( $cf, $val ) {
			...
			$product = $val ? wc_get_product( $val ) : false;
			echo $product ? esc_attr( get_the_title( $product->get_id() ) ) : esc_attr( wpsc__( 'None', 'supportcandy' ) );
		}
