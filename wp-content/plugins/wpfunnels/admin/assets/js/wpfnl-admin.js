(function($) {
    ...
    jQuery(document).ready(function($) {
        ...
        function wpfnlDatePicker() {
            $(".wpfunnel-datepicker").datepicker({
                ...
                beforeShowDay: function(date) {
                    let date1 = $.datepicker.parseDate($.datepicker._defaults.dateFormat, $("#date_from_value").val());
                    let date2 = $.datepicker.parseDate($.datepicker._defaults.dateFormat, $("#date_to_value").val());

                    let isStartDate = date1 && date.getTime() === date1.getTime();
                    let isEndDate = date2 && date.getTime() === date2.getTime();

                    return [true, (isStartDate || (date2 && date >= date1 && date <= date2)) ? "in-range" : "", isStartDate ? 'Start' : (isEndDate ? 'End' : '')];
                ...
                onSelect: function(dateText, inst) {
                    ...
                    if (!date1 || date2) {
                        ...
                        $("li.select-to-date").text($("li.select-from-date").text()).css('color', '#2D3149');
                        $("li.select-from-date").text(dateText).css('color', '#2D3149');
        ...
        var FunnelHandler = function() {
            ...
            $('.import-funnel-modal .close-modal, .import-funnel-modal .import-funnel-modal-inner').on('click', this.CloseImportFunnelModal);
            $('.import-funnel-modal .import-funnel-modal-wrapper').on('click', this.PreventImportFunnelModal);
         ...
         FunnelHandler.prototype.UploadFile = function(event) {
            ...
            if( importFile ){
                ...
                if( 'json' === fileExtension ){
                    ...
                    $('.import-funnel-modal .import-label > h4').html('Drag & Drop or <span class="primary-color">Choose file</span> to upload.');
        ...
        var funnelNameWidth = '';
         $('.funnel-name-edit').on('click', function(e) {
            ...
            $(this).parents('.steps-page__fnl-name').find('.funnel-name-input').css("width", (funnelNameWidth) + "px");
            $(this).parents('.steps-page__fnl-name').find('.funnel-name-input').val(fnl_name).show().focus();
    ...
    */
    ...
    $(document).on("click", ".order-bump-header .add-new-orderbump", function(e) {
        ...
        $('.single-order-bump').siblings().find('.order-bump-accordion-content').removeClass('show-content');
        $('.single-order-bump:last-child').find('.order-bump-accordion-content').addClass('show-content');
    });
    ...
    */
    $(document).on("click", ".drawflow .drawflow-node .single-node", function(e) {
        ...
        $(this).parent('.single-node-wrapper').siblings().find('.single-node').removeClass('im-selected-node');
    ...
    */
    function selectedFunnelCounter(){
        ...
        if(totalSelectedFunnel > 1){
            $('.funnel__single-list.list-header .bulk-action-wrapper .selected-funnel-count').text(totalSelectedFunnel + ' Funnels');
        }else {
            $('.funnel__single-list.list-header .bulk-action-wrapper .selected-funnel-count').text(totalSelectedFunnel + ' Funnel');
        }
    }

    $(".funnel__single-list.list-body input[name=funnel-list-select]").on('change', function(){
        $('.funnel-list__bulk-select.select-all-funnels input[name=funnel-list__bulk-select]').prop('checked', false);
        selectedFunnelCounter();
    });

    $(".funnel-list__bulk-select.select-all-funnels input[name=funnel-list__bulk-select]").on('change', function(){
        if ($(this).is(':checked')){
            $('.funnel__single-list.list-body input[name=funnel-list-select]').prop('checked', true);
            selectedFunnelCounter();
        }else {
            $('.funnel__single-list.list-header .bulk-action-wrapper').css('display', 'none');
            $('.funnel__single-list.list-body input[name=funnel-list-select]').prop('checked', false);
        }
    });
    ...
    */
    ...
    $(document).on("click", ".mailmint-automation .add-action > button", function(e) {
        ...
        $(this).parents('.single-decision').siblings().find('.add-action').removeClass('show-actions');
        
        $(this).parents('.automation-single-step').siblings().find('.add-action').removeClass('show-actions');
    ...
    $(document).on("click", ".single-ab-step .more-option > button", function(e) {
        e.stopPropagation();
        $(this).parents('.single-ab-step').siblings().find('.more-option').removeClass("show-dorpdown");
    ...
    */
    $(document).on("click", ".wpfnl-selectbox .selected-option", function(e) {
        ...
        $(this).parents('.single-rules').siblings().find('.wpfnl-selectbox').removeClass('show-options');
        $(this).parents('.rules').siblings().find('.wpfnl-selectbox').removeClass('show-options');

        $(this).parents('.condition-single-row').siblings().find('.wpfnl-selectbox').removeClass('show-options');
        $(this).parents('.single-condition').siblings().find('.wpfnl-selectbox').removeClass('show-options');
    ...
    */
    $(document).on("click", ".single-rules .selectable-options > li", function(e) {
        ...
        $(this).parents('.single-rules').siblings().find('.selectable-options > li').removeClass('show-options');
        $(this).parents('.rules').siblings().find('.selectable-options > li').removeClass('show-options');
    ...
    $(document).on("click", ".single-rules .submenu > li, .rules-is .selectable-options > li", function() {
        $('.single-rules .wpfnl-selectbox').removeClass("show-options");
    });
    ...
    $(document).on("click", ".wpfnl-custom-selectbox .selectbox-selected-value", function(e) {
        $('.single-rules .wpfnl-selectbox ').removeClass('show-options');
        $(this).parents('.single-rules').siblings().find('.wpfnl-custom-selectbox').removeClass('show-options');
    });
