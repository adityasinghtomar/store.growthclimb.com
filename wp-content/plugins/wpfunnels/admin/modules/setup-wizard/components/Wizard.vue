	...
	<div class="wpfnl wpfnl-setup-wizard">
		<div class="wpfnl-setup-wizard__wrapper">
            ...
            <FunnelType
				...
				:isWCInstalled="isWCInstalled"
				:isWCActive="isWCActive"
				:isCLInstalled="isCLInstalled"
			    :isCLActive="isCLActive"
            ...
            />
			<Automation
                v-if="step==='automation'"
                :wizardSlug="wizardSlug"
				:setUpType="setUpType"
				:showLoader="showLoader"
				:prevStepLink="prevStepLink"
				:isMrmInstalled="isMrmInstalled"
				:isMrmActive="isMrmActive"
                @setPluginSlug="setPluginSlug"
                @changeSetUpType="changeSetUpType"
                @changeFunnelType="changeFunnelType"
                @processSettings="processSettings"
    ...
    export default {
        ...
        data: function () {
            return {
                ...
                isMrmInstalled:  window.setup_wizard_obj.is_mrm_installed,
                ...
                isMrmActive:    window.setup_wizard_obj.is_mrm_active,
				...
				pluginSlug:     'sales' === window.setup_wizard_obj.funnel_type ? 'woocommerce' : false,
                setUpType:      'plugin',
                isMailMint:      false,
                showLoader:      false,
                isPermitted:     false,
	...
	beforeMount() {
		setTimeout(() => {
			...
			for (let i = 0; i < bodyChildren.length; i++) {
				...
				if (element.id !== 'wpfunnels_setup_wizard') {
	...
	methods: {
		...
		processSettings: function (e) {
			...
			if( this.pluginSlug === 'woocommerce' ) {
				...
				this.updateBuilderType('elementor');
			...
			}else if (this.pluginSlug === 'mail-mint') {
				if (this.isMrmInstalled === 'yes') {
					let plugins = [
						'mail-mint'
					];
					this.activatePlugins(plugins)
				} else if (this.isMrmActive === 'no') {
					this.installPlugin('mail-mint')
				}
				
			}else if (this.pluginSlug === 'gutenberg') {
				this.updateBuilderType('gutenberg');
			...
			}else if (this.pluginSlug === 'divi-builder') {
				this.updateBuilderType('divi-builder');
			}else if (this.pluginSlug === 'oxygen') {
				this.updateBuilderType('oxygen');
			}else if (this.pluginSlug === 'bricks') {
				this.updateBuilderType('bricks');
			}else if (this.pluginSlug === 'other') {
				this.updateBuilderType('other');
			}else {
		...
		activatePlugins: function(plugins) {
			this.showLoader = true;
			apiFetch({
				path: `${window.setup_wizard_obj.rest_api_url}wpfunnels/v1/settings/activate_mrm_plugins`,
				method: 'POST',
		...
		updateBuilderType: function ( builder ) {
			var that = this
			apiFetch({
				path: window.setup_wizard_obj.rest_api_url + 'wpfunnels/v1/settings/update-builder-type/',
				method: 'POST',
				data: { builder: builder }
			}).then(response => {
				if( this.pluginSlug === 'divi-builder' || this.pluginSlug === 'oxygen' || this.pluginSlug === 'bricks' || this.pluginSlug === 'other'){
					window.location.href = that.nextStepLink
				}
			}, error => {
