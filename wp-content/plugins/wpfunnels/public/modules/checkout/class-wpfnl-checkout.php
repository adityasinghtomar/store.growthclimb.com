				...
				if( isset( $product[ "id" ] ) && isset( $product[ "quantity" ] ) && ( !isset($product['funnel_id']) ||  (isset($product['funnel_id']) && (int)$funnel_id === (int)$product['funnel_id']) ) ) {
                    ...
                    if( $_product instanceof \WC_Product ) {
						...
						if( $price_type == 'original' ||  $price_type == 'sale' ){
							$custom_data = [
								'custom_price' 	=> get_post_meta($product_id, '_price', true) || 0 == get_post_meta($product_id, '_price', true) ? get_post_meta($product_id, '_price', true) : get_post_meta($product_id, '_regular_price', true),
